...
namespace MoistureUpset.Skins.Jotaro
{
    class JotaroDisplayFix : MonoBehaviour
    {
        ...
        private void Start()
        {
            var fab = AddressableLoader.LoadAsset<GameObject>("RoR2/Base/Captain/CaptainDisplay.prefab", "JotaroDisplayFix.Start");
            if (!fab)
            {
                return;
            }
        ...
        private void Update()
        {
            ...
            foreach (var smr in GetComponentsInChildren<SkinnedMeshRenderer>())
            {
                ...
                if (smr.material.mainTexture.name.ToLowerInvariant() == SkinHelper.SkinNameToSkinMeshName[JotaroCaptain.NameToken].ToLowerInvariant())
                {
                    isJotaroSkin = true;
                }
            }
